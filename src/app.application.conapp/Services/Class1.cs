...
//This call willbe updated as general to be used as service for other gsheet related operations 
public class GoogleSheetsHelper
{
    ...
    public List<ExpandoObject> GetDataFromSheet(GoogleSheetParameters googleSheetParameters)
    {
        ...
        if (values != null && values.Count > 0)
        {
            foreach (var row in values)
            {
                ...
                {
                    expandoDict.Add(columnName, row[columnCounter].ToString() ?? string.Empty);
                    columnCounter++;
                }
    ...
    public void AddCells(GoogleSheetParameters googleSheetParameters, List<GoogleSheetRow> rows)
    {
        ...
        var sheetId = GetSheetId(_sheetsService, _spreadsheetId, googleSheetParameters.SheetName ?? string.Empty);
